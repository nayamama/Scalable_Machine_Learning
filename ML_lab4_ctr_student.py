...
# TODO: Replace <FILL IN> with appropriate code
model0 = LogisticRegressionWithSGD.train(OHETrainData,  iterations = 50, step=10., miniBatchFraction=1.0, initialWeights=None, regParam=1e-6, regType='l2', intercept = True)
...
def evaluateResults(model, data):
    ...
    logLoss = (data
              ...
              .map(lambda (a,b) : computeLogLoss(a, b))
              .mean())
    return logLoss
...
def parseHashPoint(point, numBuckets):
    ...
    Returns:
        LabeledPoint: A LabeledPoint with a label (0.0 or 1.0) and a SparseVector of hashed
            features.
    """
    hashFeats = hashFunction(numBuckets, parsePoint(point), True)
    return LabeledPoint(point[0], SparseVector(numBuckets, hashFeats))
...
def computeSparsity(data, d, n):
    ...
    """
    return data.map(lambda lp:lp.features).map(lambda sparse:float(len(sparse.values))/d).mean()
