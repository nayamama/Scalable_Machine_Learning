...
def getCountsAndAverages(IDandRatingsTuple):
    ...
    averageRating = sum(IDandRatingsTuple[1])/float(numberOfRatings) 
...
movieNameWithAvgRatingsRDD = (moviesRDD
                              ...
                              .map(lambda (MovieID, (Title, (Count, AveRating))) : (AveRating, Title, Count))
                              )
...
movieLimitedAndSortedByRatingRDD = (movieNameWithAvgRatingsRDD
                                    .filter(lambda (Ave, title, count) : count > 500)
...
def computeError(predictedRDD, actualRDD):
    ...
    squaredErrorsRDD = (predictedReformattedRDD
                       ...
                       .map(lambda ((UserID, MovieID), (Rating_p, Rating_a)) : math.pow((Rating_p - Rating_a), 2)))
