...
def tokenize(string):
    ...
    """
    s = re.split(split_regex, string.lower())
    noStopWord = [word for word in s if word not in stopwords]

    return [x for x in noStopWord if x]
...
# TODO: Replace <FILL IN> with appropriate code
amazonRecToToken = amazonSmall.map(lambda data : (data[0], tokenize(data[1])))
googleRecToToken = googleSmall.map(lambda data : (data[0], tokenize(data[1])))
...
print tf(["word", "word", "dog", "cat", "dog", "dog"])
...
def idfs(corpus):
    ...
    uniqueTokens1 = corpus.flatMap(lambda doc: doc[1])
    ...
    #print tokenSumPairTuple.take(5)
    #print (tokenSumPairTuple.map(lambda (k, v) : (k, N*1.0/v) )).take(5)
...
print idfsSmall.takeOrdered(5, lambda s: s[1])[0]
...
avgSimDups = trueDupsRDD.map(lambda (a, (b,c)) : b).reduce(lambda a, b : a + b)/float(trueDupsCount)

nonDupsRDD = (sims
              .leftOuterJoin(goldStandard)
              .filter(lambda (k, v): v[1]==None))

avgSimNon = nonDupsRDD.map(lambda (k,v): v[0]).reduce(lambda v1, v2: v1+v2)/float(nonDupsRDD.count())
