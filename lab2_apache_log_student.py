...
daysWithAvg = (avgDailyReqPerHost
               .map(lambda (day, avg): day)
               .take(30)
               )

avgs = (avgDailyReqPerHost
        .map(lambda (day, avg): avg)
        .take(30)
       )
...
daysWithErrors404 = (errDateSorted
                     .map(lambda (day, error_count) : day)
                     .take(30)
                     )

errors404ByDay = (errDateSorted
                  .map(lambda (day, error_count) : error_count)
                  .take(30)
                  )
...
topErrDate = (errDateSorted
              .takeOrdered(5, lambda (day, count) : -1 * count)
              )
