...
def parsePoint(line):
    ...
    return LabeledPoint(split_str[0], split_str[1:]) 
...
def calcRMSE(labelsAndPreds):
    ...
    """
    total = (labelsAndPreds
             .map(lambda (labels, pred) : squaredError(labels, pred))
             .reduce(lambda x,y : x+y)
             )
    return (total/labelsAndPreds.count())**0.5
...
def linregGradientDescent(trainData, numIters):
    ...
    for i in range(numIters):
        ...
        labelsAndPredsTrain = trainData.map(lambda obz: getLabeledPrediction(w, obz))
        errorTrain[i] = calcRMSE(labelsAndPredsTrain)

        # Calculate the `gradient`.  Make use of the `gradientSummand` function you wrote in (3a).
        # Note that `gradient` sould be a `DenseVector` of length `d`.
        gradient = trainData.map(lambda rec: gradientSummand(w, rec)).reduce(lambda v1, v2: v1+v2)
...
def twoWayInteractions(lp):
    ...
    """
    twoWays = np.array([x*y for (x,y) in itertools.product(lp.features,repeat=2)])
    return LabeledPoint(lp.label, np.hstack([lp.features, twoWays]))
